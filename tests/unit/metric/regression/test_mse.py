...
from evalix.metric import mean_squared_error
...
def test_mean_squared_error_nan_omit() -> None:
    assert objects_are_equal(
        mean_squared_error(
...
def test_mean_squared_error_nan_omit_y_true() -> None:
    assert objects_are_equal(
        mean_squared_error(
...
def test_mean_squared_error_nan_omit_y_pred() -> None:
    assert objects_are_equal(
        mean_squared_error(
...
def test_mean_squared_error_nan_propagate() -> None:
    assert objects_are_equal(
        mean_squared_error(
            y_true=np.array([float("nan"), 2, 3, 4, 5, float("nan")]),
            y_pred=np.array([1, 2, 3, 4, float("nan"), float("nan")]),
        ...
        {"count": 6, "mean_squared_error": float("nan")},
        equal_nan=True,
    )


def test_mean_squared_error_nan_propagate_y_true() -> None:
    assert objects_are_equal(
        mean_squared_error(
            y_true=np.array([1, 2, 3, 4, 5, float("nan")]),
            y_pred=np.array([1, 2, 3, 4, 5, 0]),
...
def test_mean_squared_error_nan_propagate_y_pred() -> None:
    assert objects_are_equal(
        mean_squared_error(
            y_true=np.array([1, 2, 3, 4, 5, 0]),
            y_pred=np.array([1, 2, 3, 4, 5, float("nan")]),
...
def test_mean_squared_error_nan_raise() -> None:
...
def test_mean_squared_error_nan_raise_y_true() -> None:
    with pytest.raises(ValueError, match="'y_true' contains at least one NaN value"):
        mean_squared_error(
            y_true=np.array([1, 2, 3, 4, 5, float("nan")]),
            y_pred=np.array([1, 2, 3, 4, 5, 0]),
...
def test_mean_squared_error_nan_raise_y_pred() -> None:
